    ...
    public void initializeFilters(List<Filter> newFilters)
    {
    	...
    	for (Filter f : newFilters)
    	{
    		...
    		if (f.tags.containsName("prev"))
    			f.prevDesc = newDescriptions.stream().filter(pf -> pf.name.equals(f.tags.getString("prev").orElse(""))).collect(Collectors.toList());
    		
    		if (f.tags.containsName("next"))
    			f.nextDesc = newDescriptions.stream().filter(nf -> nf.name.equals(f.tags.getString("next").orElse(""))).collect(Collectors.toList());
    		
    		if (f.tags.containsName("bridge"))
    			f.nextDesc = newDescriptions.stream().filter(bf -> bf.name.equals(f.tags.getString("bridge").orElse(""))).collect(Collectors.toList());    		
    	}
    }
